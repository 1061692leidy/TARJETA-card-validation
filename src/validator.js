const validator = {
  isValid: function(cardNumber) {
    {
      ...
      for (let i = 0; i < reversed.length; i++) {
        // i < reversed.length" es la condición que determina cuándo se detiene el bucle. Continúa hasta que "i" sea menor que la longitud del array "reversed i++" es el incremento de "i" en 1 en cada iteración del bucle
        ...
        if (i % 2 === 1) {
          // La estructura "if" se ejecuta solo si el resultado de la comparación es verdadero, es decir, si "i % 2" es igual a 1. Esto significa que la acción dentro de la estructura "if" solo se ejecuta para los índices impares de "i" en el array "reversed"
          ...
          if (current > 9) {
            current -= 9; //current es el resultado por 2 
          }
        }
        total += current;
      }
  ...
  maskify: function(cardNumber) {
    ...
    for (let i = 0; i < cardNumber.length - 4; i++) {
      i < cardNumber.length - 4 //" es la condición de parada del bucle. El bucle continuará hasta que "i" sea mayor o igual a "cardNumber.length - 4, i++" es el incremento de "i" en cada iteración del bucle
